...
class UserExtended(models.Model):
    ...
    is_driver = models.BooleanField('Conductor', default=False, null=True)
    photo_profile = models.ImageField('Foto de Perfil', upload_to='fotos_de_perfil', null=True, blank=True)
    license_plate = models.CharField('placa', max_length=6, null=True, blank=True)
    is_active = models.BooleanField('Conductor', default=True)
...
class Service(models.Model):
    ...
    client = models.ForeignKey(UserExtended, related_name='service_client', on_delete=models.CASCADE)
    driver = models.ForeignKey(UserExtended, related_name='service_drive', on_delete=models.CASCADE, null=True)
    ...
    disability = models.BooleanField('Conductor', default=False)
    charge = models.BooleanField('Conductor', default=False)
    pet = models.BooleanField('Conductor', default=False)
    passengers = models.BooleanField('Conductor', default=False)
    ...
    cancel_reason = models.CharField('Motivo cancelacion servicio', max_length=100, default="")
...
class Comment(models.Model):
    client = models.ForeignKey(UserExtended, related_name='comment_client', on_delete=models.CASCADE)
    driver = models.ForeignKey(UserExtended, related_name='comment_drive', on_delete=models.CASCADE)
